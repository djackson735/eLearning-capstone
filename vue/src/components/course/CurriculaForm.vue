...
export default {
  ...
  methods: {
    submitForm() {
      ...
      if (this.curriculaID === 0) {
        coursesService.addCurricula(this.courseId, newCurricula).then(response => {
        ...
        newCurricula.id = this.curriculaID;
        newCurricula.avatar = this.curricula.avatar;
        coursesService.updateCurricula(newCurricula).then(response => {
            if (response.status === 200) {
              this.$router.push(`/course/${newCurricula.courseId}`);
            }
          })
          .catch(error => {
  ...
  created() {
    if (this.curriculaID != 0) {
      coursesService.getCurricula(this.curriculaID).then(response => {
